...
var Spark, SparkTest, TAG, Renderer, patch, Patcher;
...
var idStringify;                                                                                   // 906
                                                                                                   // 907
if (typeof LocalCollection !== 'undefined') {                                                      // 908
  idStringify = function (id) {                                                                    // 909
    if (id === null)                                                                               // 910
      return id;                                                                                   // 911
    else                                                                                           // 912
      return LocalCollection._idStringify(id);                                                     // 913
  };                                                                                               // 914
} else {                                                                                           // 915
  idStringify = function (id) { return id; };                                                      // 916
}                                                                                                  // 917
                                                                                                   // 918
Spark.list = function (cursor, itemFunc, elseFunc) {                                               // 919
  elseFunc = elseFunc || function () { return ''; };                                               // 920
                                                                                                   // 921
  // Create a level of indirection around our cursor callbacks so we                               // 922
  // can change them later                                                                         // 923
  var callbacks = {};                                                                              // 924
  var observerCallbacks = {};                                                                      // 925
  _.each(["addedBefore", "removed", "movedBefore", "changed"], function (name) {                   // 926
    observerCallbacks[name] = function () {                                                        // 927
      return callbacks[name].apply(null, arguments);                                               // 928
    };                                                                                             // 929
  });                                                                                              // 930
                                                                                                   // 931
  // Get the current contents of the cursor.                                                       // 932
                                                                                                   // 933
  var itemDict = new OrderedDict(idStringify);                                                     // 934
  _.extend(callbacks, {                                                                            // 935
    addedBefore: function (id, item, before) {                                                     // 936
      var doc = EJSON.clone(item);                                                                 // 937
      doc._id = id;                                                                                // 938
      var elt = {doc: doc, liveRange: null};                                                       // 939
      itemDict.putBefore(id, elt, before);                                                         // 940
    }                                                                                              // 941
  });                                                                                              // 942
  var handle = cursor.observeChanges(observerCallbacks);                                           // 943
                                                                                                   // 944
  // Get the renderer, if any                                                                      // 945
  var renderer = currentRenderer.get();                                                            // 946
  var maybeAnnotate = renderer ?                                                                   // 947
        _.bind(renderer.annotate, renderer) :                                                      // 948
    function (html) { return html; };                                                              // 949
                                                                                                   // 950
  // Templates should have access to data and methods added by the                                 // 951
  // transformer, but observeChanges doesn't transform, so we have to do                           // 952
  // it here.                                                                                      // 953
  //                                                                                               // 954
  // NOTE: this is a little bit of an abstraction violation. Ideally,                              // 955
  // the only thing Spark should know about Minimongo is the contract of                           // 956
  // observeChanges. In theory, anything that implements observeChanges                            // 957
  // could be passed to Spark.list. But meh.                                                       // 958
  var transformedDoc = function (doc) {                                                            // 959
    if (cursor.getTransform && cursor.getTransform())                                              // 960
      return cursor.getTransform()(EJSON.clone(doc));                                              // 961
    return doc;                                                                                    // 962
  };                                                                                               // 963
                                                                                                   // 964
  // Render the initial contents. If we have a renderer, create a                                  // 965
  // range around each item as well as around the list, and save them                              // 966
  // off for later.                                                                                // 967
  var html = '';                                                                                   // 968
  var outerRange;                                                                                  // 969
  if (itemDict.empty())                                                                            // 970
    html = elseFunc();                                                                             // 971
  else {                                                                                           // 972
    itemDict.forEach(function (elt) {                                                              // 973
        html += maybeAnnotate(                                                                     // 974
          itemFunc(transformedDoc(elt.doc)),                                                       // 975
          ANNOTATION_LIST_ITEM,                                                                    // 976
          function (range) {                                                                       // 977
            elt.liveRange = range;                                                                 // 978
          });                                                                                      // 979
    });                                                                                            // 980
  }                                                                                                // 981
  var stopped = false;                                                                             // 982
  var cleanup = function () {                                                                      // 983
    handle.stop();                                                                                 // 984
    stopped = true;                                                                                // 985
  };                                                                                               // 986
  html = maybeAnnotate(html, ANNOTATION_LIST, function (range) {                                   // 987
    if (! range) {                                                                                 // 988
      // We never ended up on the screen (caller discarded our return                              // 989
      // value)                                                                                    // 990
      cleanup();                                                                                   // 991
    } else {                                                                                       // 992
      outerRange = range;                                                                          // 993
      outerRange.finalize = cleanup;                                                               // 994
    }                                                                                              // 995
  });                                                                                              // 996
                                                                                                   // 997
  // No renderer? Then we have no way to update the returned html and                              // 998
  // we can close the observer.                                                                    // 999
  if (! renderer)                                                                                  // 1000
    cleanup();                                                                                     // 1001
                                                                                                   // 1002
  // Called by `removed` and `moved` in order to cause render callbacks on                         // 1003
  // parent landmarks.                                                                             // 1004
  // XXX This is not the final solution.  1) This code should be unified                           // 1005
  // with the code in scheduleOnscreenSetup.  2) In general, lists are                             // 1006
  // going to cause a lot of callbacks (one per collection callback).                              // 1007
  // Maybe that will make sense if we give render callbacks subrange info.                         // 1008
  var notifyParentsRendered = function () {                                                        // 1009
    var walk = outerRange;                                                                         // 1010
    while ((walk = findParentOfType(ANNOTATION_LANDMARK, walk)))                                   // 1011
      walk.rendered.call(walk.landmark);                                                           // 1012
  };                                                                                               // 1013
                                                                                                   // 1014
  var later = function (f) {                                                                       // 1015
    Deps.afterFlush(function () {                                                                  // 1016
      if (! stopped)                                                                               // 1017
        withEventGuard(f);                                                                         // 1018
    });                                                                                            // 1019
  };                                                                                               // 1020
                                                                                                   // 1021
  // The DOM update callbacks.                                                                     // 1022
  _.extend(callbacks, {                                                                            // 1023
    addedBefore: function (id, fields, before) {                                                   // 1024
      later(function () {                                                                          // 1025
        var doc = EJSON.clone(fields);                                                             // 1026
        doc._id = id;                                                                              // 1027
        var frag = Spark.render(_.bind(itemFunc, null, transformedDoc(doc)));                      // 1028
        DomUtils.wrapFragmentForContainer(frag, outerRange.containerNode());                       // 1029
        var range = makeRange(ANNOTATION_LIST_ITEM, frag);                                         // 1030
                                                                                                   // 1031
        if (itemDict.empty()) {                                                                    // 1032
          Spark.finalize(outerRange.replaceContents(frag));                                        // 1033
        } else if (before === null) {                                                              // 1034
          itemDict.lastValue().liveRange.insertAfter(frag);                                        // 1035
        } else {                                                                                   // 1036
          itemDict.get(before).liveRange.insertBefore(frag);                                       // 1037
        }                                                                                          // 1038
        itemDict.putBefore(id, {doc: doc, liveRange: range}, before);                              // 1039
      });                                                                                          // 1040
    },                                                                                             // 1041
                                                                                                   // 1042
    removed: function (id) {                                                                       // 1043
      later(function () {                                                                          // 1044
        if (itemDict.first() === itemDict.last()) {                                                // 1045
          var frag = Spark.render(elseFunc);                                                       // 1046
          DomUtils.wrapFragmentForContainer(frag, outerRange.containerNode());                     // 1047
          Spark.finalize(outerRange.replaceContents(frag));                                        // 1048
        } else                                                                                     // 1049
          Spark.finalize(itemDict.get(id).liveRange.extract());                                    // 1050
                                                                                                   // 1051
        itemDict.remove(id);                                                                       // 1052
                                                                                                   // 1053
        notifyParentsRendered();                                                                   // 1054
      });                                                                                          // 1055
    },                                                                                             // 1056
                                                                                                   // 1057
    movedBefore: function (id, before) {                                                           // 1058
      later(function () {                                                                          // 1059
        var frag = itemDict.get(id).liveRange.extract();                                           // 1060
        if (before === null) {                                                                     // 1061
          itemDict.lastValue().liveRange.insertAfter(frag);                                        // 1062
        }                                                                                          // 1063
        else {                                                                                     // 1064
          itemDict.get(before).liveRange.insertBefore(frag);                                       // 1065
        }                                                                                          // 1066
        itemDict.moveBefore(id, before);                                                           // 1067
        notifyParentsRendered();                                                                   // 1068
      });                                                                                          // 1069
    },                                                                                             // 1070
                                                                                                   // 1071
    changed: function (id, fields) {                                                               // 1072
      later(function () {                                                                          // 1073
        var elt = itemDict.get(id);                                                                // 1074
        if (!elt)                                                                                  // 1075
          throw new Error("Unknown id for changed: " + id);                                        // 1076
        applyChanges(elt.doc, fields);                                                             // 1077
        Spark.renderToRange(elt.liveRange,                                                         // 1078
                            _.bind(itemFunc, null, transformedDoc(elt.doc)));                      // 1079
      });                                                                                          // 1080
    }                                                                                              // 1081
  });                                                                                              // 1082
                                                                                                   // 1083
  return html;                                                                                     // 1084
                                                                                                   ...
                                                                                                   // 1086
/******************************************************************************/                   // 1087
/* Labels and landmarks                                                       */                   // 1088
/******************************************************************************/                   // 1089
                                                                                                   // 1090
var nextLandmarkId = 1;                                                                            // 1091
                                                                                                   // 1092
Spark.Landmark = function () {                                                                     // 1093
  this.id = nextLandmarkId++;                                                                      // 1094
  this._range = null; // will be set when put onscreen                                             // 1095
};                                                                                                 // 1096
                                                                                                   // 1097
_.extend(Spark.Landmark.prototype, {                                                               // 1098
  firstNode: function () {                                                                         // 1099
    return this._range.firstNode();                                                                // 1100
  },                                                                                               // 1101
  lastNode: function () {                                                                          // 1102
    return this._range.lastNode();                                                                 // 1103
  },                                                                                               // 1104
  find: function (selector) {                                                                      // 1105
    var r = this._range;                                                                           // 1106
    return DomUtils.findClipped(r.containerNode(), selector,                                       // 1107
                                r.firstNode(), r.lastNode());                                      // 1108
  },                                                                                               // 1109
  findAll: function (selector) {                                                                   // 1110
    var r = this._range;                                                                           // 1111
    return DomUtils.findAllClipped(r.containerNode(), selector,                                    // 1112
                                   r.firstNode(), r.lastNode());                                   // 1113
  },                                                                                               // 1114
  hasDom: function () {                                                                            // 1115
    return !! this._range;                                                                         // 1116
  }                                                                                                // 1117
});                                                                                                // 1118
                                                                                                   // 1119
Spark.UNIQUE_LABEL = ['UNIQUE_LABEL'];                                                             // 1120
                                                                                                   // 1121
// label must be a string.                                                                         // 1122
// or pass label === null to not drop a label after all (meaning that                              // 1123
// this function is a noop)                                                                        // 1124
//                                                                                                 // 1125
Spark.labelBranch = function (label, htmlFunc) {                                                   // 1126
  var renderer = currentRenderer.get();                                                            // 1127
  if (! renderer || label === null)                                                                // 1128
    return htmlFunc();                                                                             // 1129
                                                                                                   // 1130
  if (label === Spark.UNIQUE_LABEL)                                                                // 1131
    label = Random.id();                                                                           // 1132
                                                                                                   // 1133
  renderer.currentBranch.pushLabel(label);                                                         // 1134
  var html = htmlFunc();                                                                           // 1135
  var occupied = renderer.currentBranch.getNotes().occupied;                                       // 1136
  renderer.currentBranch.popLabel();                                                               // 1137
                                                                                                   // 1138
  if (! occupied)                                                                                  // 1139
    // don't create annotation if branch doesn't contain any landmarks.                            // 1140
    // if this label isn't on an element-level HTML boundary, then that                            // 1141
    // is certainly the case.                                                                      // 1142
    return html;                                                                                   // 1143
                                                                                                   // 1144
  return renderer.annotate(                                                                        // 1145
    html, ANNOTATION_LABEL, { label: label });                                                     // 1146
                                                                                                   // 1147
  // XXX what happens if the user doesn't use the return value, or                                 // 1148
  // doesn't use it directly, eg, swaps the branches of the tree                                   // 1149
  // around? "that's an error?" the result would be that the apparent                              // 1150
  // branch path of a landmark at render time would be different from                              // 1151
  // its apparent branch path in the actual document. seems like the                               // 1152
  // answer is to have labelBranch not drop an annotation, and keep                                // 1153
  // the branch label info outside of the DOM in a parallel tree of                                // 1154
  // labels and landmarks (likely similar to the one we're already                                 // 1155
  // keeping?) a little tricky since not every node in the label tree                              // 1156
  // is actually populated with a landmark? (though we could change                                // 1157
  // that I guess -- they would be landmarks without any specific DOM                              // 1158
  // nodes?)                                                                                       // 1159
};                                                                                                 // 1160
                                                                                                   // 1161
Spark.createLandmark = function (options, htmlFunc) {                                              // 1162
  var renderer = currentRenderer.get();                                                            // 1163
  if (! renderer) {                                                                                // 1164
    // no renderer -- create and destroy Landmark inline                                           // 1165
    var landmark = new Spark.Landmark;                                                             // 1166
    options.created && options.created.call(landmark);                                             // 1167
    var html = htmlFunc(landmark);                                                                 // 1168
    options.destroyed && options.destroyed.call(landmark);                                         // 1169
    return html;                                                                                   // 1170
  }                                                                                                // 1171
                                                                                                   // 1172
  // Normalize preserve map                                                                        // 1173
  var preserve = {};                                                                               // 1174
  if (_.isArray(options.preserve))                                                                 // 1175
    _.each(options.preserve, function (selector) {                                                 // 1176
      preserve[selector] = true;                                                                   // 1177
    });                                                                                            // 1178
  else                                                                                             // 1179
    preserve = options.preserve || {};                                                             // 1180
  for (var selector in preserve)                                                                   // 1181
    if (typeof preserve[selector] !== 'function')                                                  // 1182
      preserve[selector] = function () { return true; };                                           // 1183
                                                                                                   // 1184
  renderer.currentBranch.mark('occupied');                                                         // 1185
  var notes = renderer.currentBranch.getNotes();                                                   // 1186
  var landmark;                                                                                    // 1187
  if (notes.originalRange) {                                                                       // 1188
    if (notes.originalRange.superceded)                                                            // 1189
      throw new Error("Can't create second landmark in same branch");                              // 1190
    notes.originalRange.superceded = true; // prevent destroyed(), second match                    // 1191
    landmark = notes.originalRange.landmark; // the old Landmark                                   // 1192
  } else {                                                                                         // 1193
    landmark = new Spark.Landmark;                                                                 // 1194
    if (options.created) {                                                                         // 1195
      // Run callback outside the current Spark.isolate's deps context.                            // 1196
      Deps.nonreactive(function () {                                                               // 1197
        options.created.call(landmark);                                                            // 1198
      });                                                                                          // 1199
    }                                                                                              // 1200
  }                                                                                                // 1201
  notes.landmark = landmark;                                                                       // 1202
                                                                                                   // 1203
  var html = htmlFunc(landmark);                                                                   // 1204
  return renderer.annotate(                                                                        // 1205
    html, ANNOTATION_LANDMARK, function (range) {                                                  // 1206
      if (! range) {                                                                               // 1207
        // annotation not used                                                                     // 1208
        options.destroyed && options.destroyed.call(landmark);                                     // 1209
        return;                                                                                    // 1210
      }                                                                                            // 1211
                                                                                                   // 1212
      _.extend(range, {                                                                            // 1213
        preserve: preserve,                                                                        // 1214
        constant: !! options.constant,                                                             // 1215
        rendered: options.rendered || function () {},                                              // 1216
        destroyed: options.destroyed || function () {},                                            // 1217
        landmark: landmark,                                                                        // 1218
        finalize: function () {                                                                    // 1219
          if (! this.superceded) {                                                                 // 1220
            this.landmark._range = null;                                                           // 1221
            this.destroyed.call(this.landmark);                                                    // 1222
          }                                                                                        // 1223
        }                                                                                          // 1224
      });                                                                                          // 1225
                                                                                                   // 1226
      landmark._range = range;                                                                     // 1227
      renderer.landmarkRanges.push(range);                                                         // 1228
      // Help GC avoid an actual memory leak (#1157) by nulling the                                // 1229
      // `renderer` local variable, which holds data structures about                              // 1230
      // the preservation and patching performed during this rendering                             // 1231
      // pass, including references to the old LiveRanges.  If                                     // 1232
      // `renderer` is retained by the LiveRange we initialize here,                               // 1233
      // it creates a chain linking the new LiveRanges to the                                      // 1234
      // renderer, to the old LiveRanges, to the old renderer, etc.                                // 1235
      //                                                                                           // 1236
      // The reason the new LiveRange might retains `renderer` has to                              // 1237
      // do with how V8 implements closures.  V8 considers                                         // 1238
      // `range.finalize` to close over `renderer`, even though it                                 // 1239
      // doesn't use it.  Because `renderer` is used by *some* nested                              // 1240
      // closure, it apparently is retained by all nested closures as                              // 1241
      // part of `Spark.createLandmark`'s function context.                                        // 1242
      renderer = null;                                                                             // 1243
    });                                                                                            // 1244
};                                                                                                 // 1245
                                                                                                   // 1246
SparkTest.getEnclosingLandmark = function (node) {                                                 // 1247
  var range = findRangeOfType(ANNOTATION_LANDMARK, node);                                          // 1248
  return range ? range.landmark : null;                                                            // 1249
};                                                                                                 // 1250
                                                                                                   // 1251
/////////////////////////////////////////////////////////////////////////////////////////////////////
...
Meteor.renderList = function (cursor, itemFunc, elseFunc) {                                        // 11
  return Spark.render(function () {                                                                // 12
    return Spark.list(cursor, function (item) {                                                    // 13
      return Spark.labelBranch(item._id || null, function () {                                     // 14
        return Spark.isolate(_.bind(itemFunc, null, item));                                        // 15
      });                                                                                          // 16
    }, function () {                                                                               // 17
      return elseFunc ? Spark.isolate(elseFunc) : '';                                              // 18
    });                                                                                            // 19
  });                                                                                              // 20
};                                                                                                 // 21
                                                                                                   // 22
...
//# sourceMappingURL=c9939ae1b11c764e0ce2709f852e72f6382467e6.map
